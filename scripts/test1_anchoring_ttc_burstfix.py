...
#   Plots: scripts/results/test1_burstfix_boxplot.png
#          scripts/results/test1_burstfix_ecdf_burst.png
#
...
TIMEOUT_SEC = 120                         # Increased timeout for safety
WARMUP_MILESTONES = 3
...
def confirm_ttc_until(block_id: str, start_ts: float,
    ...
    """Poll until referenced or do one single check."""
    ms_idx = None
    if single_check:
        try:
...
def run_burst(tier_commits: int, run_id: int, pattern_label: str):
    ...
    while commits_done < tier_commits:
        ...
        # Round-robin confirmation checks
        unresolved = {p["blockId"]: p for p in pending if p["blockId"]}
        start_time = time.time()
        while unresolved and (time.time() - start_time) < (TIMEOUT_SEC * 2):
            for bid, p in list(unresolved.items()):
                ...
                if ok:
                    ...
                    print(f"✅ commit {commits_done}/{tier_commits} TTC={ttc:.2f}s ms={ms_idx} Δ→MS={p['to_next_ms']:.1f}s msInt={ms_int}")
